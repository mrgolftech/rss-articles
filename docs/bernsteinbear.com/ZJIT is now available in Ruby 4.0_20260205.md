...
_Originally published on[Rails At Scale](https://railsatscale.com/2025-12-24-launch-zjit/)._

ZJIT is a new just-in-time (JIT) Ruby compiler built into the reference Ruby implementation, [YARV](https://en.wikipedia.org/wiki/YARV), by the same compiler group that brought you YJIT. We (Aaron Patterson, Aiden Fox Ivey, Alan Wu, Jacob Denbeaux, Kevin Menard, Max Bernstein, Maxime Chevalier-Boisvert, Randy Stauner, Stan Lo, and Takashi Kokubun) have been working on ZJIT since the beginning of this year.
...
It’s been a long time since we gave an official update on ZJIT. Things are going well. We’re excited to share our progress with you. We’ve done a lot [since May](/blog/merge-zjit/).
...
> For example, we can [constant-fold](https://en.wikipedia.org/wiki/Constant_folding) numeric operations. Because we also have a (small, limited) inliner borrowed from YJIT, we can constant-fold the entirety of `add` down to `3`—and still handle redefinitions of `one`, `two`, `Integer#+`, …
>     
>     
>     def one
>       1
...
Back in May, we had four people working full-time on the compiler. **Now,** we have more internally at Shopify—and also more from the community! We have had several interested people reach out, learn about ZJIT, and successfully land complex changes. For this reason, we have opened up [a chat room](https://zjit.zulipchat.com) to discuss and improve ZJIT.

### A cool graph visualization tool

You _have to_ check out our intern Aiden’s [integration of Iongraph into ZJIT](https://railsatscale.com/2025-11-19-adding-iongraph-support/). Now we have clickable, zoomable, scrollable graphs of all our functions and all our optimization passes. It’s great!
...
We have public performance numbers for a selection of macro- and micro-benchmarks on [rubybench](https://rubybench.github.io/). Here is a screenshot of what those per-benchmark graphs look like. The Y axis is speedup multiplier vs the interpreter and the X axis is time. Higher is better:

![](/assets/img/zjit-benchmark.png) A line chart of ZJIT performance on railsbench--represented as a speedup multiplier when compared to the interpreter--improving over time, passing interpreter performance, catching up to YJIT.
...
While Ruby now ships with ZJIT compiled into the binary by default, it is not _enabled_ by default at run-time. Due to performance and stability, YJIT is still the default compiler choice in Ruby 4.0.
...
We devoted a lot of our resources this year to developing ZJIT. While we did not spend much time on YJIT (outside of a great [allocation speed up](https://railsatscale.com/2025-05-21-fast-allocations-in-ruby-3-5/)), YJIT isn’t going anywhere soon.
...
This blog is [open source](https://github.com/tekknolagi/tekknolagi.github.com). See an error? Go ahead and [propose a change](https://github.com/tekknolagi/tekknolagi.github.com/edit/main/_posts/2025-12-24-launch-zjit.md "Help improve _posts/2025-12-24-launch-zjit.md").   
