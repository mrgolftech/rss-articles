...
**日期:** Sat, 31 Jan 2026 00:00:00 +0000
...
I write a lot of [Makefiles](https://en.wikipedia.org/wiki/Make_\(software\)). I use it not as a command runner but as an ad-hoc build system for small projects, typically for compiling Markdown documents and their dependencies. Like so:

![A build graph for a document. A central node `doc.md` represents the Markdown source. Two outgoing arrows point to `doc.html` and `doc.pdf`, representing the output format. A chain through `graph.dot`, `graph.png`, and `doc.md` represets how a Graphviz .dot file can be rendered to PNG. A chain through `data.csv`, `plot.py`, `plot.png`, and `doc.md` represents using a Python script to make a plot from a CSV file.](/assets/content/some-data-should-be-code/graph.png)
...
(I could never remember the [automatic variable](https://www.gnu.org/software/make/manual/html_node/Automatic-Variables.html) syntax until I made [flashcards](https://github.com/eudoxia0/flashcards/blob/aefae3ed874627201dbcedec045095779691d323/Cards/make.md) for them.)
...
Fortunately this exists: it’s called [doit](https://pydoit.org/), but it’s not widely known.

* * *

A lot of things are like Makefiles: data that should be lifted one level up to become code.

Consider [CloudFormation](https://en.wikipedia.org/wiki/AWS_CloudFormation). Nobody likes writing those massive YAML files by hand, so AWS introduced [CDK](https://en.wikipedia.org/wiki/AWS_Cloud_Development_Kit), which is literally just a library1 of classes that represent AWS resources. Running a CDK program emits CloudFormation YAML as though it were an assembly language for infrastructure. And so you get type safety, modularity, abstraction, conditionals and loops, all for free.

Consider [GitHub Actions](https://docs.github.com/en/actions). How much better off would we be if, instead of writing the workflow-job-step tree by hand, we could just have a single Python script, executed on push, whose output is the GitHub Actions YAML-as-assembly? So you might write:
...
# Footnotes
  ...
  1. Or rather, a polyglot collection of libraries, one per language, like [Pulumi](https://en.wikipedia.org/wiki/Pulumi). ↩
...
---|---  
Previous |  [ Letting Claude Play Text Adventures ](/article/letting-claude-play-text-adventures)  
Next |  [ Julia ](/fiction/julia)  
