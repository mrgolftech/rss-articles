...
Even though I criticize [Beads](https://github.com/steveyegge/beads) for its implementation, giving an agent a to-do list is genuinely useful. The `/todos` command brings up all items stored in `.pi/todos` as markdown files. Both the agent and I can manipulate them, and sessions can claim tasks to mark them as in progress.
