...
in tags [nix](/posts/tags/nix/) [ai](/posts/tags/ai/)

[![Edit Icon](/Bilder/pen-square-solid.svg)](https://github.com/stapelberg/hugo/edit/master/content/posts/2026-02-01-coding-agent-microvm-nix/index.markdown)
...
The [`microvm.nix` project](https://github.com/microvm-nix/microvm.nix) makes it easy to create such VMs on NixOS, and this article shows you how I like to set up my VMs.

## See also

If you haven’t heard of NixOS before, check out [the NixOS Wikipedia page](https://en.wikipedia.org/wiki/NixOS) and [nixos.org](https://nixos.org/). I [spoke about why I switched to Nix in 2025](/talks/#2025) and have published a [few blog posts about Nix](/posts/tags/nix/).

For understanding the threat model of AI agents, read [Simon Willison’s “The lethal trifecta for AI agents: private data, untrusted content, and external communication” (June 2025)](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/). This article’s approach to working with the threat model is to remove the “private data” part from the equation.

If you want to learn about the whole field of sandboxing, check out [Luis Cardoso’s “A field guide to sandboxes for AI” (Jan 2026)](https://www.luiscardoso.dev/blog/sandboxes-for-ai). I will not be comparing different solutions in this article, I will just show you one possible path.

And lastly, maybe you’re not in the mood to build/run sandboxing infrastructure yourself. Good news: Sandboxing is a hot topic and there are many commercial offerings popping up that address this need. For example, David Crawshaw and Josh Bleecher Snyder (I know both from the Go community) recently launched [exe.dev](https://blog.exe.dev/meet-exe.dev), an agent-friendly VM hosting service. Another example is [Fly.io, who launched Sprites](https://fly.io/blog/code-and-let-live/).
...
The `microvm-base.nix` module takes these parameters and declares:
  ...
  * Network settings: I like using [`systemd-networkd(8)`](https://manpages.debian.org/systemd-networkd.8) and [`systemd-resolved(8)`](https://manpages.debian.org/systemd-resolved.8) .
...
Then, SSH into the VM (perhaps in a [`tmux(1)`](https://manpages.debian.org/tmux.1) session) and run Claude (or your Coding Agent of choice) without permission prompts in the shared workspace directory:
...
[![Claude Code in “bypass permissions” mode](https://michael.stapelberg.ch/posts/2026-02-01-coding-agent-microvm-nix/2026-01-28-neofetch-featured_hu_d06e7aa7176833b3.png)](https://michael.stapelberg.ch/posts/2026-02-01-coding-agent-microvm-nix/2026-01-28-neofetch-featured.png)

## Creating VMs with Claude

After going through the process of setting up a MicroVM once, it becomes tedious.

I was curious if [Claude Skills](https://code.claude.com/docs/en/skills) could help with a task like this. Skills are markdown files that instruct Claude to do certain steps in certain situations.
...
This actually mirrors my experience with Coding Agents: I don’t feel like they’re _automatically_ making existing tasks more efficient, I feel that they make things possible that were previously out of reach (similar to [Jevons paradox](https://en.wikipedia.org/wiki/Jevons_paradox)).
