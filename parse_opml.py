...
<opml version="2.0">
  ...
  <body>
    <outline text="Blogs" title="Blogs">
      ...
      <outline type="rss" text="chiark.greenend.org.uk/~sgtatham" title="chiark.greenend.org.uk/~sgtatham" xmlUrl="https://www.chiark.greenend.org.uk/~sgtatham/quasiblog/feed.xml" htmlUrl="https://www.chiark.greenend.org.uk/~sgtatham"/>
...
for outline in root.findall(".//outline[@type='rss']"):
    ...
    blogs.append((name, rss_url, html_url))

# Existing blogs from directory
existing = [
    "anildash.com", "antirez.com", "aresluna.org", "berthub.eu",
    "blog.jim-nielsen.com", "borretti.me", "buttondown.com",
    "chadnauseam.com", "construction-physics.com", "daringfireball.net",
    "devblogs.microsoft.com", "dfarq.homeip.net", "entropicthoughts.com",
    "evanhahn.com", "experimental-history.com", "filfre.net",
    "garymarcus.substack.com", "grantslatton.com", "idiallo.com",
    "jeffgeerling.com", "joanwestenberg.com", "johndcook.com",
    "krebsonsecurity.com", "lucumr.pocoo.org", "martinalderson.com",
    "micahflee.com", "michael.stapelberg.ch", "miguelgrinberg.com",
    "nesbitt.io", "paulgraham.com", "pluralistic.net", "righto.com",
    "seangoedecke.com", "shkspr.mobi", "simonwillison.net",
    "steveblank.com", "tedium.co", "terriblesoftware.org",
    "troyhunt.com", "wheresyoured.at", "worksonmymachine.substack.com",
    "xania.org"
]

# Find missing blogs
missing = []
for name, rss_url, html_url in blogs:
    if name not in existing:
        missing.append((name, rss_url, html_url))

print(f"Total blogs in OPML: {len(blogs)}")
print(f"Existing blogs: {len(existing)}")
print(f"Missing blogs: {len(missing)}")
print("\nMissing blogs:")
for name, rss_url, html_url in missing:
    print(f"{name} | {rss_url}")

# Write missing blogs to a file
with open('/root/openclaw/gw/docs/rss/missing_blogs.txt', 'w') as f:
    for name, rss_url, html_url in missing:
        f.write(f"{name}|{rss_url}|{html_url}\n")